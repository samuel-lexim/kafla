...
;(function($) {
	adminLayer = function(base)
	{
		...
		$('#opt-desktop').on('click','#sslider-layer-add',function(){
			sangarCanvas.addBox('desktop',377);
		})
		...
		$('#opt-mobile').on('click','#sslider-layer-add',function(){
			sangarCanvas.addBox('mobile',377);
		})		
		...
		/**
		 * Modal Available Shortcode
		 */
		$('#layer-modal-available-shortcode').dialog({
	        autoOpen: false,
	        modal: true,
	        draggable: false,
	        resizable: false,
		...
		$('#layer-modal-image').dialog({
	        ...
	        height: 475,
			buttons: {
				'Available Shortcode': function(){
		            $('#layer-modal-available-shortcode').dialog('open');
		        },
		        'Cancel': function(){
		            $(this).dialog('close');
		        },
		    ...
		    create:function () {
		        ...
		        // available shortcode
		        $(this).closest(".ui-dialog").children('.ui-dialog-buttonpane')
		...
		$('#layer-modal-text').dialog({
		    ...
		    open:function () {
            	...
            	if($(this).data('text') != '')
            	{
            		$(this).find('#add-layer-text').val($(this).data('text'));
            	}
		...
		/**
		 * Modal Add Post Object Content
		 */
		$('.sangar-layer').on('click','.add-post-content',function(){
			$current = $(this).parents('.layer');
			var newHeight = $current.height() == 330 ? 90 : 330;
		...
		$('.sangar-layer').on('click','.add-post-content-container ul li',function(){
			var layer = $(this).parents('.layer');
			var id = layer.data('id');
			var type = layer.data('type');
			...
			// set pre-text
			modal.data('text',$(this).data('text'));
			modal.data('custom-title',$(this).html());

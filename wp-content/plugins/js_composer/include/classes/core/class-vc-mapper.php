		...
		if ( isset( $this->element_activities[ $tag ] ) ) {
			...
			foreach ($this->element_activities[ $tag ] as $activity) {
				if (is_array($activity)) {
					...
					switch ( $activity[0]) {
						case 'drop_param':
							WPBMap::dropParam( $activity[1]['name'], $activity[1]['attribute_name'] );
							break;
						case 'add_param':
							WPBMap::addParam( $activity[1]['name'], $activity[1]['attribute'] );
							break;
						case 'mutate_param':
							WPBMap::mutateParam( $activity[1]['name'], $activity[1]['attribute'] );
							break;
						case 'drop_shortcode':
							WPBMap::dropShortcode( $activity[1]['name'] );
							break;
						case 'modify':
							WPBMap::modify( $activity[1]['name'], $activity[1]['setting_name'], $activity[1]['value'] );
							break;
					}
				}
			}

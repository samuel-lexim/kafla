	...
	switch($type) {
		...
		case 'custom': {
			...
			foreach(qtranxf_getSortedLanguages() as $language) {
				...
				if($language == $q_config['language']) $classes[] = 'active';
				if ($language == 'en')
					echo '<li class="'.implode(' ', $classes).'"><a href="' . qtranxf_convertURL($url, $language, false, true) . '" title="'.$alt.'">ENGLISH</a></li>'.PHP_EOL;
				if ($language == 'ko')
					echo '<li class="'.implode(' ', $classes).'"><a href="http://www.kafla.org/ko/" title="'.$alt.'">한국어</a></li>'.PHP_EOL;
			}
		} break;
	}
	echo '</ul><div class="qtranxs_widget_end"></div>'.PHP_EOL;

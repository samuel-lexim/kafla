...
class SiennaMikadoImport {

    public $message = "";
    public $attachments = false;
    function SiennaMikadoImport() {
    ...
    function init_mkd_import() {
        if(isset($_REQUEST['import_option'])) {
            ...
            if($import_option == 'content'){
                ...
                $this->import_menus('menus.txt');
                $this->import_settings_pages('settingpages.txt');
                $this->message = esc_html__("Content imported successfully", "sienna");
            }
        }
    }
    ...
    public function import_content($file){
        if (!class_exists('WP_Importer')) {
            ...
            if(is_wp_error($returned_value)){
                $this->message = esc_html__("An Error Occurred During Import", "sienna");
            }
            else {
                $this->message = esc_html__("Content imported successfully", "sienna");
            }
            ...
            $this->message = esc_html__("Error loading files", "sienna");
        }
    }
    ...
    public function import_widgets($file, $file2){
        ...
        $this->message = esc_html__("Widgets imported successfully", "sienna");
    }
    ...
    public function import_custom_sidebars($file){
        ...
        $this->message = esc_html__("Custom sidebars imported successfully", "sienna");
    }

    public function import_options($file){
        $options = $this->file_options($file);
        update_option( 'mkd_options_sienna', $options);
        $this->message = esc_html__("Options imported successfully", "sienna");
    }
    ...
    public function file_options($file){
        ...
        /*if ( file_exists($file_for_import) ) {
            ...
            $this->message = esc_html__("File doesn't exist", "sienna");
        }*/
    ...
    function mkd_admin_import() {
        if (mkd_core_theme_installed()) {
            ...
            $this->pagehook = add_submenu_page(
                ...
                'sienna_mikado_theme_menu'.$slug,                // The ID used to bind submenu items to this menu
                array(sienna_mikado_framework()->getSkin(), 'renderImport')
            );

            add_action('admin_print_scripts-'.$this->pagehook, 'sienna_mikado_enqueue_admin_scripts');
            add_action('admin_print_styles-'.$this->pagehook, 'sienna_mikado_enqueue_admin_styles');
...
function mkd_init_import_object(){
    global $sienna_mikado_import_object;
    $sienna_mikado_import_object = new SiennaMikadoImport();
}

add_action('init', 'mkd_init_import_object');

if(!function_exists('sienna_mikado_dataImport')){
    function sienna_mikado_dataImport(){
        global $sienna_mikado_import_object;

        if ($_POST['import_attachments'] == 1)
            $sienna_mikado_import_object->attachments = true;
        else
            $sienna_mikado_import_object->attachments = false;

        $folder = "sienna/";
        if (!empty($_POST['example']))
            $folder = $_POST['example']."/";

        $sienna_mikado_import_object->import_content($folder.$_POST['xml']);

        die();
    }

    add_action('wp_ajax_mkd_dataImport', 'sienna_mikado_dataImport');
}

if(!function_exists('sienna_mikado_widgetsImport')){
    function sienna_mikado_widgetsImport(){
        global $sienna_mikado_import_object;
        ...
        $sienna_mikado_import_object->import_widgets($folder.'widgets.txt',$folder.'custom_sidebars.txt');

        die();
    }

    add_action('wp_ajax_mkd_widgetsImport', 'sienna_mikado_widgetsImport');
}

if(!function_exists('sienna_mikado_optionsImport')){
    function sienna_mikado_optionsImport(){
        global $sienna_mikado_import_object;
        ...
        $sienna_mikado_import_object->import_options($folder.'options.txt');

        die();
    }

    add_action('wp_ajax_mkd_optionsImport', 'sienna_mikado_optionsImport');
}

if(!function_exists('sienna_mikado_otherImport')){
    function sienna_mikado_otherImport(){
        global $sienna_mikado_import_object;
        ...
        $sienna_mikado_import_object->import_options($folder.'options.txt');
        $sienna_mikado_import_object->import_widgets($folder.'widgets.txt',$folder.'custom_sidebars.txt');
        $sienna_mikado_import_object->import_menus($folder.'menus.txt');
        $sienna_mikado_import_object->import_settings_pages($folder.'settingpages.txt');

        die();
    }

    add_action('wp_ajax_mkd_otherImport', 'sienna_mikado_otherImport');
}
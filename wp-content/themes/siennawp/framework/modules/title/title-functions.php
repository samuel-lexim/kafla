...
if(!function_exists('sienna_mikado_get_title')) {
	/**
	 * Loads title area HTML
	 */
	function sienna_mikado_get_title() {
		$id = sienna_mikado_get_page_id();

		extract(sienna_mikado_title_area_height());
		extract(sienna_mikado_title_area_background());
		...
		if(($meta_temp = get_post_meta($id, "mkdf_show_title_area_meta", true)) !== "") {
			...
			$show_title_area = sienna_mikado_options()->getOptionValue('show_title_area') == 'yes' ? true : false;
		}
		...
		if(($meta_temp = get_post_meta($id, "mkdf_title_area_type_meta", true)) !== "") {
			...
			$type = sienna_mikado_options()->getOptionValue('title_area_type');
		}
		...
		if(($meta_temp = get_post_meta($id, "mkdf_title_area_enable_breadcrumbs_meta", true)) !== "") {
			...
			$enable_breadcrumbs = sienna_mikado_options()->getOptionValue('title_area_enable_breadcrumbs') == 'yes' ? true : false;
		}
		...
		$parameters = apply_filters('sienna_mikado_title_area_height_params', $parameters);

		sienna_mikado_get_module_template_part('templates/title', 'title', '', $parameters);
	}
}

if(!function_exists('sienna_mikado_get_title_text')) {
	/**
	 * Function that returns current page title text. Defines sienna_mikado_title_text filter
	 ...
	 */
	function sienna_mikado_get_title_text() {
		...
		if(is_tag()) {
			...
			$title = single_term_title("", false).esc_html__(' Tag', 'sienna');
		...
		elseif(is_author()) {
			...
			$title = esc_html__('Author:', 'sienna')." ".get_the_author();
		...
		elseif(is_search()) {
			...
			$title = esc_html__('Search', 'sienna');
		} //is current page 404?
		elseif(is_404()) {
			//is 404 title text set in theme options?
			if(sienna_mikado_options()->getOptionValue('404_title') != "") {
				//get it from options
				$title = sienna_mikado_options()->getOptionValue('404_title');
			} else {
				//get default 404 page title
				$title = esc_html__('404 - Page not found', 'sienna');
			}
		} //is WooCommerce installed and is shop or single product page?
		elseif(sienna_mikado_is_woocommerce_installed() && (is_shop() || is_singular('product'))) {
		...
		elseif(sienna_mikado_is_woocommerce_installed() && (is_product_category() || is_product_tag())) {
			global $wp_query;
		...
		elseif(is_archive()) {
			$title = esc_html__('Archive', 'sienna');
		} //current page is regular page
		else {
			$title = get_the_title($id);
		}

		$title = apply_filters('sienna_mikado_title_text', $title);

		return $title;
	}
}

if(!function_exists('sienna_mikado_title_text')) {
	/**
	 * Function that echoes title text.
	 *
	 * @see sienna_mikado_get_title_text()
	 */
	function sienna_mikado_title_text() {
		echo sienna_mikado_get_title_text();
	}
}

if(!function_exists('sienna_mikado_subtitle_text')) {
	/**
	 * Function that echoes subtitle text.
	 *
	 */
	function sienna_mikado_subtitle_text() {
		$id       = sienna_mikado_get_page_id();
		$subtitle = '';
...
if(!function_exists('sienna_mikado_custom_breadcrumbs')) {
	/**
	 ...
	 * @see is_front_page()
	 * @see is_category()
	 * @see sienna_mikado_is_product_category()
	 * @see get_search_query()
	 */
	function sienna_mikado_custom_breadcrumbs() {
		global $post, $wp_query;

		$output      = "";
		$homeLink    = esc_url(home_url('/'));
		$pageid      = sienna_mikado_get_page_id();
		$bread_style = "";
		...
		$delimiter   = '<span class="mkdf-delimiter" '.sienna_mikado_get_inline_style($bread_style).'>&nbsp;/&nbsp;</span>'; // delimiter between crumbs
		$home        = get_bloginfo('name'); // text for the 'Home' link
		$showCurrent = 1; // 1 - show current post/page title in breadcrumbs, 0 - don't show
		$before      = '<span class="mkdf-current" '.sienna_mikado_get_inline_style($bread_style).'>'; // tag before the current crumb
		$after       = '</span>'; // tag after the current crumb

		if(is_home() && !is_front_page()) {
			$output = '<div class="mkdf-breadcrumbs"><div class="mkdf-breadcrumbs-inner"><a '.sienna_mikado_get_inline_style($bread_style).' href="'.$homeLink.'">'.$home.'</a>'.$delimiter.' <a '.sienna_mikado_get_inline_style($bread_style).' href="'.$homeLink.'">'.get_the_title($pageid).'</a></div></div>';

		} elseif(is_home()) {
			$output = '<div class="mkdf-breadcrumbs"><div class="mkdf-breadcrumbs-inner">'.$before.$home.$after.'</div></div>';
		} elseif(is_front_page()) {
			if($showOnHome == 1) {
				$output = '<div class="mkdf-breadcrumbs"><div class="mkdf-breadcrumbs-inner"><a '.sienna_mikado_get_inline_style($bread_style).' href="'.$homeLink.'">'.$home.'</a></div></div>';
			}
		} else {

			$output .= '<div class="mkdf-breadcrumbs"><div class="mkdf-breadcrumbs-inner"><a '.sienna_mikado_get_inline_style($bread_style).' href="'.$homeLink.'">'.$home.'</a>'.$delimiter;

			if(is_category() || sienna_mikado_is_product_category()) {
				...
				$output .= $before.esc_html__('Search results for ', 'sienna').'"'.get_search_query().'"'.$after;

			} elseif(is_day()) {
				$output .= '<a '.sienna_mikado_get_inline_style($bread_style).' href="'.get_year_link(get_the_time('Y')).'">'.get_the_time('Y').'</a>'.$delimiter;
				$output .= '<a '.sienna_mikado_get_inline_style($bread_style).' href="'.get_month_link(get_the_time('Y'), get_the_time('m')).'">'.get_the_time('F').'</a>'.$delimiter;
				...
				$output .= '<a '.sienna_mikado_get_inline_style($bread_style).' href="'.get_permalink($parent).'">'.$parent->post_title.'</a>';
				if($showCurrent == 1) {
				...
				while($parent_id) {
					...
					$breadcrumbs[] = '<a '.sienna_mikado_get_inline_style($bread_style).' href="'.get_permalink($page->ID).'">'.get_the_title($page->ID).'</a>';
				...
				$output .= $before.esc_html__('Posts tagged ', 'sienna').'"'.single_tag_title('', false).'"'.$after;

			} elseif(is_author()) {
				global $authordata;
				$output .= $before.esc_html__('Articles posted by ', 'sienna').$authordata->display_name.$after;
			...
			if(get_query_var('paged')) {
				...
				$output .= $before." (".esc_html__('Page', 'sienna').' '.get_query_var('paged').")".$after;
...
if(!function_exists('sienna_mikado_get_title_area_height')) {

	/**
	 * Function that returns title height
	 **/
	function sienna_mikado_get_title_area_height() {
		$id         = sienna_mikado_get_page_id();
		...
		} elseif(sienna_mikado_options()->getOptionValue('title_area_height') !== '') {
			$title_height = sienna_mikado_options()->getOptionValue('title_area_height');
		}

		return apply_filters('sienna_mikado_title_area_height', $title_height);
	}
}

if(!function_exists('sienna_mikado_get_title_content_padding')) {
	/**
	 * Function that returns title content pading
	 **/

	function sienna_mikado_get_title_content_padding() {
		return apply_filters('sienna_mikado_title_content_padding', 0);
	}
}


if(!function_exists('sienna_mikado_title_area_height')) {
	/**
	 * Function that returns title height and padding to be applied in template
	 **/

	function sienna_mikado_title_area_height() {
		$id                            = sienna_mikado_get_page_id();
		$title_height_and_padding      = array();
		$title_height                  = sienna_mikado_get_title_area_height();
		$header_height_padding         = sienna_mikado_get_title_content_padding();
		...
		if(($meta_temp = get_post_meta($id, "mkdf_title_area_vertial_alignment_meta", true)) !== '') {
			...
			$title_vertical_alignment = sienna_mikado_options()->getOptionValue('title_area_vertial_alignment');;
		}
		...
		$title_height_and_padding['title_height'] = 'height:'.( (int) $title_height + (int) $header_height_padding).'px;';
...
if(!function_exists('sienna_mikado_title_area_background')) {
	/**
	 * Function that returns title background style be applied in template
	 **/

	function sienna_mikado_title_area_background() {
		$id                           = sienna_mikado_get_page_id();
		$show_title_img               = true;
		...
		if(($meta_temp = get_post_meta($id, "mkdf_title_area_background_color_meta", true)) != "") {
			...
			$background_color = sienna_mikado_options()->getOptionValue('title_area_background_color');
		}
		...
		if(($meta_temp = get_post_meta($id, "mkdf_title_area_background_image_meta", true)) != "") {
			...
			$background_image = sienna_mikado_options()->getOptionValue('title_area_background_image');
		}

		//check for background image width
		$background_image_width = "";
		if($background_image !== '') {
			$background_image_width_dimensions_array = sienna_mikado_get_image_dimensions($background_image);

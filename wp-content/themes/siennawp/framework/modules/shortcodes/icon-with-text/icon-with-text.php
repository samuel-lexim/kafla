<?php
namespace Sienna\Modules\Shortcodes\IconWithText;

use Sienna\Modules\Shortcodes\Lib\ShortcodeInterface;

/**
 * Class IconWithText
 * @package Sienna\Modules\Shortcodes\IconWithText
 */
class IconWithText implements ShortcodeInterface {
    ...
    public function vcMap() {
        vc_map(array(
            ...
            'params'                    => array_merge(
                sienna_mikado_icon_collections()->getVCParamsArray(array(), '', true),
                array(
                    array(
                        'type'       => 'attach_image',
                        'heading'    => 'Custom Icon',
                    ...
                    array(
                        'type'        => 'dropdown',
                        'heading'     => 'Show Line Between Icons?',
                        'param_name'  => 'line_param',
                        'value'       => array(
                            'Yes' => 'yes',
                            'No'  => 'no'
                        ),
                        'save_always' => true,
                        'admin_label' => true,
                        'dependency'  => array('element' => 'icon_position', 'value' => array('left', 'right')),
                        'description' => 'Show line between icons.',
                    ),
                    array(
                        'type'       => 'colorpicker',
                        'heading'    => 'Line Between Icons Color',
                    ...
                    array(
                        ...
                        'value'       => array_flip(sienna_mikado_get_text_transform_array(true)),
                        'save_always' => true,
                        'group'       => 'Text Settings'
                    ),
                    array(
    ...
    public function render($atts, $content = null) {
        ...
        return sienna_mikado_get_shortcode_module_template_part('templates/iwt', 'icon-with-text', $params['icon_position'], $params);
    }
    ...
    private function getHolderClasses($params) {
        ...
        if($this->hasLineBetweenIcons($params)) {
            $classes[] = 'mkdf-has-line-between-icons';
        }
    ...
    private function getContentStyles($params) {
        ...
        if($params['icon_position'] == 'left' && !empty($params['text_left_padding'])) {
            $styles[] = 'padding-left: '.sienna_mikado_filter_px($params['text_left_padding']).'px';
        }

        if($params['icon_position'] == 'right' && !empty($params['text_right_padding'])) {
            $styles[] = 'padding-right: '.sienna_mikado_filter_px($params['text_right_padding']).'px';
        }
    ...
    private function hasLineBetweenIcons($params) {
        return $params['line_param'] === 'yes';
    }

    private function getBarStyles($params) {
        $styles = array();

        if(!empty($params['padding_bottom'])) {
            $styles[] = 'padding-bottom: '.sienna_mikado_filter_px($params['padding_bottom']).'px';
        }

        if(!empty($params['padding_top'])) {
            $styles[] = 'padding-top: '.sienna_mikado_filter_px($params['padding_top']).'px';
        }

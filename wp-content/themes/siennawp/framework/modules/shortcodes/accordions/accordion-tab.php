...
namespace Sienna\Modules\AccordionTab;
...
class AccordionTab implements ShortcodeInterface {
	...
	public function vcMap() {
		if(function_exists('vc_map')) {
			vc_map(array(
				"name"                    => esc_html__('Mikado Accordion Tab', 'sienna'),
				"base"                    => $this->base,
				...
				'params'                  => array_merge(
					...
					array(
						array(
							'type'        => 'textfield',
							'heading'     => esc_html__('Title', 'sienna'),
							'param_name'  => 'title',
							'admin_label' => true,
							'value'       => esc_html__('Section', 'sienna'),
							'description' => esc_html__('Enter accordion section title.', 'sienna')
						),
						array(
							'type'        => 'el_id',
							'heading'     => esc_html__('Section ID', 'sienna'),
							'param_name'  => 'el_id',
							'description' => sprintf(esc_html__('Enter optional row ID. Make sure it is unique, and it is valid as w3c specification: %s (Must not have spaces)', 'sienna'), '<a target="_blank" href="http://www.w3schools.com/tags/att_global_id.asp">'.esc_html__('link', 'sienna').'</a>'),
						),
					)
				)
			));
	...
	public function render($atts, $content = null) {
		...
		$output .= sienna_mikado_get_shortcode_module_template_part('templates/accordion-template', 'accordions', '', $params);

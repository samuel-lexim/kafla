<?php
namespace Sienna\Modules\CallToAction;
...
class CallToAction implements ShortcodeInterface {
	...
	public function vcMap() {
		...
		$call_to_action_button_IconCollections = sienna_mikado_icon_collections()->iconCollections;
		...
		vc_map(array(
			...
			'params'                    => array_merge(
				...
				sienna_mikado_icon_collections()->getVCParamsArray(array(
					'element' => 'type',
				...
				array(
					...
					array(
						...
						'value'       => array_flip(sienna_mikado_get_btn_types(true)),
						'save_always' => true,
						'admin_label' => true,
					...
					array(
						...
						'value'       => array_flip(sienna_mikado_get_btn_hover_animation_types(true)),
						'save_always' => true,
						'admin_label' => true,
						'dependency'  => array('element' => 'show_button', 'value' => array('yes')),
						'group'       => 'Design Options',
					...
					array(
						...
						'value'       => array_merge(array('No Icon' => ''), sienna_mikado_icon_collections()->getIconCollectionsVC()),
						'save_always' => true,
						'dependency'  => array('element' => 'show_button', 'value' => array('yes'))
					)
				),
	...
	public function render($atts, $content = null) {
		...
		$args = array_merge($args, sienna_mikado_icon_collections()->getShortcodeParams(), $call_to_action_icons_form_fields);
		...
		$html = sienna_mikado_get_shortcode_module_template_part('templates/call-to-action-template', 'calltoaction', '', $params);
	...
	private function getCallToActionStyles($params) {
		...
		if($params['box_border_color'] != '') {
			...
			if($params['box_border_width'] !== '') {
				$border_width = sienna_mikado_filter_px($params['box_border_width']);
			}
	...
	private function getCallToActionIcon($params) {
		...
		$icon                                   = sienna_mikado_icon_collections()->getIconCollectionParamNameByKey($params['icon_pack']);
		$iconStyles                             = array();
		$iconStyles['icon_attributes']['style'] = $this->getIconStyles($params);
		$call_to_action_icon                    = '';
		if(!empty($params[$icon])) {
			$call_to_action_icon = sienna_mikado_icon_collections()->renderIcon($params[$icon], $params['icon_pack'], $iconStyles);
		}
	...
	private function getButtonParameters($params) {
		...
		if(!empty($params['button_icon_pack'])) {
			...
			$iconPackName                       = sienna_mikado_icon_collections()->getIconCollectionParamNameByKey($params['button_icon_pack']);

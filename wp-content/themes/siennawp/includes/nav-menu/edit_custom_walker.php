 ...
 */
class SiennaMikado_Walker_Nav_Menu_Edit_Custom extends Walker_Nav_Menu {
	/**
	...
	function start_el(&$output, $item, $depth = 0, $args = array(), $id = 0) {
		...
		if(!empty($item->_invalid)) {
			...
			$title = sprintf(esc_html__('%s (Invalid)', 'sienna'), $item->title);
		} elseif(isset($item->post_status) && 'draft' == $item->post_status) {
			$classes[] = 'pending';
			/* translators: %s: title of menu item in draft status */
			$title = sprintf(esc_html__('%s (Pending)', 'sienna'), $item->title);
		}
			...
			<dt class="menu-item-handle">
                ...
                <span class="item-controls">
                    ...
                    <span class="item-order hide-if-js">
                        ...
                        ?>" class="item-move-up"><abbr title="<?php esc_attr_e('Move up', 'sienna'); ?>">&#8593;</abbr></a>
                        |
                        <a href="<?php
                        echo esc_url(wp_nonce_url(
	                        add_query_arg(
                        ...
                        ?>" class="item-move-down"><abbr title="<?php esc_attr_e('Move down', 'sienna'); ?>">&#8595;</abbr></a>
                    </span>
                    <a class="item-edit" id="edit-<?php echo esc_attr($item_id); ?>" title="<?php esc_attr_e('Edit Menu Item', 'sienna'); ?>" href="<?php
                    ...
                    ?>"><?php esc_html_e('Edit Menu Item', 'sienna'); ?></a>
                </span>
			</dt>
		</dl>
		...
		<div class="menu-item-settings" id="menu-item-settings-<?php echo esc_attr($item_id); ?>">
			...
			<p class="description description-thin">
				<label for="edit-menu-item-title-<?php echo esc_attr($item_id); ?>">
					<?php esc_html_e('Navigation Label', 'sienna'); ?><br/>
			...
			<p class="description description-thin">
				<label for="edit-menu-item-attr-title-<?php echo esc_attr($item_id); ?>">
					<?php esc_html_e('Title Attribute', 'sienna'); ?><br/>
			...
			<p class="field-link-target description">
				<label for="edit-menu-item-target-<?php echo esc_attr($item_id); ?>">
					...
					<?php esc_html_e('Open link in a new window/tab', 'sienna'); ?>
				</label>
			</p>

			<p class="field-css-classes description description-thin">
				<label for="edit-menu-item-classes-<?php echo esc_attr($item_id); ?>">
					<?php esc_html_e('CSS Classes (optional)', 'sienna'); ?><br/>
			...
			<p class="field-xfn description description-thin">
				<label for="edit-menu-item-xfn-<?php echo esc_attr($item_id); ?>">
					<?php esc_html_e('Link Relationship (XFN)', 'sienna'); ?><br/>
			...
			<p class="field-description description description-wide">
				<label for="edit-menu-item-description-<?php echo esc_attr($item_id); ?>">
					<?php esc_html_e('Description', 'sienna'); ?><br/>
					<textarea id="edit-menu-item-description-<?php echo esc_attr($item_id); ?>" class="widefat edit-menu-item-description" rows="3" cols="20" name="menu-item-description[<?php echo esc_attr($item_id); ?>]"><?php echo esc_html($item->description); // textarea_escaped ?></textarea>
					<span class="description"><?php esc_html_e('The description will be displayed in the menu if the current theme supports it.', 'sienna'); ?></span>
				</label>
			</p>
			<?php
			...
			<p class="field-custom description description-thin description-thin-custom">
				<label for="edit-menu-item-anchor-<?php echo esc_attr($item_id); ?>">
					<?php esc_html_e('Anchor', 'sienna'); ?><br/>
			...
			<p class="field-custom description description-wide">
				...
				<label for="edit-menu-item-nolink-<?php echo esc_attr($item_id); ?>">
					...
					<?php esc_html_e("Don't link", 'sienna'); ?>
				</label>
			</p>
			...
			<p class="field-custom description description-wide">
				...
				<label for="edit-menu-item-hide-<?php echo esc_attr($item_id); ?>">
					...
					<?php esc_html_e("Don't show", 'sienna'); ?>
				</label>
			</p>

			<p class="field-custom description description-thin description-thin-custom">
				<label for="edit-menu-item-type-menu-<?php echo esc_attr($item_id); ?>">
					<?php esc_html_e('Type', 'sienna'); ?><br/>
			...
			<p class="field-custom description description-thin description-thin-custom">
				<label for="edit-menu-item-wide-position-<?php echo esc_attr($item_id); ?>">
					<?php esc_html_e('Wide menu position', 'sienna'); ?><br/>
			...
			<p class="field-custom description description-wide">
				<label for="edit-menu-item-background-image-<?php echo esc_attr($item_id); ?>">
					<?php esc_html_e('Background image for Dropdown', 'sienna'); ?><br/>
			...
			<?php
			$iconCollections = sienna_mikado_icon_collections()->getIconCollectionsEmpty();
			...
			if(is_array($iconCollections) && count($iconCollections)) {
				...
				<p class="field-custom description description-thin description-thin-custom">
					<label for="edit-menu-item-icon-pack-<?php echo esc_attr($item_id); ?>">
						<?php esc_html_e('Icon Pack', 'sienna'); ?><br/>
						...
						<br/><?php esc_html_e('Only with "default" & "wide with icons" menu types', 'sienna'); ?>
					</label>
				</p>

				<?php
				$icon_data_attr = 'menu_item_icon_'.$item_id;
				$collection_obj = sienna_mikado_icon_collections()->getIconCollection($item->icon_pack);

				?>
				<p class="field-custom description description-thin description-thin-custom mkdf-icon-select-holder">
					<label for="edit-menu-item-icon-<?php echo esc_attr($item_id); ?>">
						<?php esc_html_e('Icon', 'sienna'); ?><br/>
			...
			<p class="field-custom description description-thin description-thin-custom" style="clear:both;">
				<label for="edit-menu-item-featured-icon-<?php echo esc_attr($item_id); ?>">
					<?php esc_html_e('Featured icon', 'sienna'); ?><br/>
			...
			<p class="field-custom description description-wide">
				<label for="edit-menu-item-sidebar-<?php echo esc_attr($item_id); ?>">
					<?php esc_html_e('Custom widget area', 'sienna'); ?><br/>
					...
					<br/><?php esc_html_e('Only with "wide & wide with icons" menu type', 'sienna'); ?>
				</label>
			</p>
			<?php
			...
			<div class="menu-item-actions description-wide submitbox">
				<?php if('custom' != $item->type && $original_title !== false) : ?>
					<p class="link-to-original">
						<?php printf(esc_html__('Original: %s', 'sienna'), '<a href="'.esc_attr($item->url).'">'.esc_html($original_title).'</a>'); ?>
					</p>
				<?php endif; ?>
				...
				)); ?>"><?php esc_html_e('Remove', 'sienna'); ?></a> <span class="meta-sep"> | </span>
				<a class="item-cancel submitcancel" id="cancel-<?php echo esc_attr($item_id); ?>" href="<?php echo esc_url(add_query_arg(array(
					'edit-menu-item' => $item_id,
					'cancel'         => time()
				), remove_query_arg($removed_args, admin_url('nav-menus.php'))));
				?>#menu-item-settings-<?php echo esc_attr($item_id); ?>"><?php esc_html_e('Cancel', 'sienna'); ?></a>
			</div>
